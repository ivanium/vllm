    ...
    def forward(
        ...
        if self.enable_sp:
            if self.ffn.use_mega_moe:
                # MegaMoE output is already fully reduced (routed experts via
                # EP combine, shared experts reduced internally), so just take
                # back the local token shard; below-threshold forwards need no
                # collective at all.
                if is_sp_sharded:
                    x = sequence_parallel_chunk(x)
            elif is_sp_sharded:
                # FusedMoE returns an un-reduced per-rank partial: `num_tokens`
                # meets the sp_threshold, so we do reduce-scatter.
                x = tensor_model_parallel_reduce_scatter(x, dim=0)
            else:
                # `num_tokens` does not meet the sp_threshold, so we do all-reduce.
                x = tensor_model_parallel_all_reduce(x)
