...
class MooncakeStoreWorker:
    ...
    def __init__(
        ...
        # Pool of load-receive threads; loads are round-robined across them so
        # concurrent requests transfer in parallel instead of serializing
        # behind one thread. Size via VLLM_MOONCAKE_LOAD_RECV_THREADS.
    ...
    def get_finished(
        ...
        for request in meta.requests:
            ...
            # Round-robin across the receive-thread pool so concurrent loads
            # run in parallel rather than serializing behind one thread.
