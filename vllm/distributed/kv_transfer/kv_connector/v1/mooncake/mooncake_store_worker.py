...
class MooncakeStoreWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        """注册 GPU KV Cache 张量并启动后台传输线程。

        此方法在 GPU KV Cache 分配完毕后由框架调用（profiling 阶段之后）。

        核心工作：
          1. 检测 KV Cache 内存布局（FlashAttn K/V-first vs FlashInfer blocks-first）。
          2. 将 GPU 内存区域注册到 MooncakeDistributedStore（使其可被远端 RDMA 访问）。
          3. 记录每个 layer/segment 的 base_addr 和 block_len，供传输线程使用。
          4. 启动 KVCacheStoreSendingThread（仅 producer/both 角色）和
             KVCacheStoreRecvingThread（所有角色均需加载）。

        内存布局检测原理：
          - 通过比较各维度的 byte stride 与 page_size_bytes 判断外层维度。
          - page_size_bytes = storage.nbytes() // num_blocks（单个物理 block 的字节数）。
          - stride(d) * element_size > page_size_bytes → d 是块外的段维度（K/V 维度）。
          - FlashAttn/ROCm：shape = (2, num_blocks, ...) → outer_dims = [0]，
            需要分别记录 K segment 和 V segment 的 base_addr。
          - FlashInfer/MLA：shape = (num_blocks, ...) → outer_dims = []，
            整个 tensor 视为一个连续内存段。
        """
