...
class OffloadingConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        # Wait for all loads to complete before model forward pass starts.
        # This is necessary because the loads are async (using separate CUDA streams)
        # and the model needs the loaded KV cache data to be ready.
...
class OffloadingConnectorWorker:
    ...
    def __init__(
        ...
        # job_id -> (req_id, group_id, is_store)
        self._jobs: dict[int, tuple[ReqId, GroupId, bool]] = {}

        # req_id -> group_id -> active load job ID
        self._load_jobs: dict[ReqId, dict[GroupId, int]] = defaultdict(dict)

        # req_id -> group_id -> set(active store job IDs)
        self._store_jobs: dict[ReqId, dict[GroupId, set[int]]] = defaultdict(
    ...
    def start_load_kv(self, metadata: OffloadingConnectorMetadata):
        """Start loading KV for all groups."""
        for req_id, group_transfers in metadata.reqs_to_load.items():
            for group_id, transfer_spec in group_transfers.items():
                job_id = self._generate_job_id()
                self._jobs[job_id] = (req_id, group_id, False)
                self._load_jobs[req_id][group_id] = job_id
                assert self.worker.transfer_async(job_id, transfer_spec)

    def wait_for_pending_loads(self):
        """Wait for all pending load jobs to complete.

        This blocks until all CPU->GPU transfers initiated by start_load_kv
        have completed. This is necessary because the loads use async CUDA
        streams and the model forward pass needs the data to be ready.
        """
        if not self._load_jobs:
            return

        # Synchronize CUDA to wait for all async transfers to complete.
        # This is simpler and more efficient than polling get_finished().
        torch.cuda.synchronize()

        # Clear load job tracking - the actual job cleanup happens in get_finished()
        # when the worker reports job completion. We just need to clear _load_jobs
        # so we don't track them as "pending" anymore.
        self._load_jobs.clear()

    def start_store_kv(self, metadata: OffloadingConnectorMetadata):
        """Start storing KV for all groups."""
        for req_id, group_transfers in metadata.reqs_to_store.items():
            for group_id, transfer_spec in group_transfers.items():
                job_id = self._generate_job_id()
                self._jobs[job_id] = (req_id, group_id, True)
                self._store_jobs[req_id][group_id].add(job_id)
                assert self.worker.transfer_async(job_id, transfer_spec)
