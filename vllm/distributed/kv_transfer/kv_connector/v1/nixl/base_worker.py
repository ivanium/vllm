...
class NixlBaseConnectorWorker:
    ...
    def _validate_remote_agent_handshake(
        ...
        if (
            ...
            if any(
                ...
                raise RuntimeError(
                    base_err
                    + "is not supported with sliding-window attention groups."
                )
        ...
        for req_id in done_recving:
            ...
            if (
                ...
                for g, local_group in enumerate(meta.local_physical_block_ids):
                    ...
                    if windowed and _is_full_attention_spec(
    ...
    def _apply_prefix_caching(
        ...
        and legacy metadata keep the older heuristics: end-trim remote on an
        ...
        if not self._has_mamba:
            ...
            for i, remote_group in enumerate(remote_block_ids):
                ...
                if num_local_blocks == 0 and num_remote_blocks == 0:
                ...
                if _is_ssm_spec(self._group_spec_types[i]):
                    ...
                    # Pair kernel blocks by token offset: local blocks start
                    # at the local prefix hit, remote blocks at token 0.
                    ...
                    local_block_ids[i] = local_block_ids[i][:count]
                    remote_block_ids[i] = remote_group[start : start + count]
                elif (
                    self._physical_blocks_per_logical_kv_block
                    == remote_physical_per_logical
                    ...
                    # blocks per side (plus one extra local block for the
