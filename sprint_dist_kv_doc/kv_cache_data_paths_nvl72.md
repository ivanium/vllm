...
## 11. P→D Transfer Latency Roofline Analysis (Kimi K2.5, measured)

> Integrated from `sprint_dist_kv_results/kimi_k2.5_decode_trace_deep_analysis_cn.md` §8. Measured bandwidth and roofline gap for path ⑥ (remote GPU HBM via NixlConnector) under Kimi K2.5 + 70K prompt.

### 11.1 Hardware Parameters

| Link | Bandwidth | Notes |
|------|-----------|-------|
| NVLink 5th gen (intra-rack GPU-GPU) | 900 GB/s (unidir) | 72-GPU all-to-all, 1.8 TB/s bidirectional |
| 4 × 200Gb NIC (RDMA/RoCE) | **100 GB/s** (4 × 25 GB/s) | mlx5_1~4, used for cross-node PD transfers |
| NVLink-C2C (GPU-CPU) | 450 GB/s (unidir) | Used by Mooncake CPU offload |

### 11.2 KV Cache Volume (Kimi MLA, 70K tokens)
...
### 11.3 Measured vs. Theoretical

| | Value | Source |
|---|---|---|
| **Measured transfer time** | **66.7 ms** per TP rank | NVTX `nixl_read_blocks_for_req` |
| **Measured sub-transfer** | 3.1 ms, 1.2 MB, 62 descriptors | Nixl metrics |
| **Measured throughput** | 385 MB/s (per sub-transfer) | Nixl metrics |

#### Roofline Comparison

| Data volume assumption | NVLink (900 GB/s) | 4×200Gb NIC (100 GB/s) | Measured | Gap vs NIC |
|-----------|-------------------|----------------------|------|-----------|
| Interpretation A: 66 MiB (62 blocks) | 0.07 ms | 0.65 ms | 66.7 ms | **103x** |
| **Interpretation B: 2.28 GiB (2188 blocks)** | **2.5 ms** | **22.9 ms** | **66.7 ms** | **2.9x** |

**Interpretation B is more reasonable**:
- Nixl transfers ~2.28 GiB KV cache per TP rank for the 70K request
- Effective throughput **34.3 GB/s**, NIC utilization **34%**
- ~3x gap from theoretical 100 GB/s

#### Gap Analysis

Possible causes of the 2.9x gap:
1. **Nixl serialization overhead**: 2,188 blocks split into ~35 batches (62 per batch) with serial RDMA READs, 3.1 ms overhead per batch
2. **Small-block inefficiency**: each sub-transfer is only 1.2 MB; RDMA small messages are latency-dominated
3. **UCX stack overhead**: UCX → verbs → NIC DMA, μs-level overhead per layer
4. **TP sync points**: the 4 TP ranks may not transfer fully in parallel due to barrier waits

#### Nixl's share of E2E latency (c=1, 70K)

```
vllm-bench results (c=1, 70K input, 300 output, 2 prompts):
  TTFT:  2,540 ms (mean)
  TPOT:  52.6 ms (mean)
  E2EL:  ~18 s (including 300 output tokens)

Nixl transfer: 66.7 ms (NVTX)
Transfer share of TTFT: 66.7 / 2540 = 2.6%
```

**At c=1, Nixl P→D transfer is not the bottleneck.** The 2.5 s TTFT is dominated by the 70K-token chunked prefill (~9 chunks × 8192 tokens). At c=4/8, concurrent transfers may change this ratio.

#### Room for Improvement

If effective throughput can be raised from 34 GB/s to 70+ GB/s (NIC utilization 70%+):
- Transfer time drops from 66.7 ms to ~33 ms
- TTFT reduction for a 70K request: ~33 ms

Key levers:
- **Larger batch size**: more blocks per transfer, amortizing per-transfer overhead
- **Pipeline transfer**: prefill starts transferring as soon as each chunk finishes, without waiting for the full prompt
- **Multi-NIC parallelism**: verify all 4 NICs are active (observable via `--nic-metrics=hf`)
