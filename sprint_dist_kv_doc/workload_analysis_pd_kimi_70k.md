# Kimi K2.5 NVFP4 PD 分离推理系统负载分析报告
...
- 唯一部分 → 需要计算 7,000 tokens
- **仅需计算 7,000 tokens (10%)，理论 prefix cache hit rate 90%**
...
1. **Turn 1 冷启动惩罚**: 所有并发度下 Turn 1 的 TTFT 显著高于后续轮次
   ...
   - 这直接验证了 90% prefix cache hit 的效果
...
**验证**: 90% prefix hit 设计 (15% global + 75% conversation) 在实际测试中得到了验证。从 70K tokens 的完整 prefill 缩减到仅 7K unique tokens 的计算，是 TTFT 大幅下降的根本原因。
...
- 机器文档: `vllm/sprint_dist_kv_docs/doc.md`
