...
## 11. P→D 传输延迟 Roofline 分析（Kimi K2.5 实测）

> 本节整合自 `sprint_dist_kv_results/kimi_k2.5_decode_trace_deep_analysis_cn.md` 第 8 章。给出路径 ⑥（远程 GPU HBM via NixlConnector）在 Kimi K2.5 + 70K prompt 下的实测带宽与 roofline gap 分析。
...
### 11.2 KV Cache 数据量（Kimi MLA, 70K tokens）
