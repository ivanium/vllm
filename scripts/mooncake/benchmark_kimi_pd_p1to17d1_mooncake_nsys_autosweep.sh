...
CONCURRENCY_PER_PREFILL="${CONCURRENCY_PER_PREFILL:-16}"
...
RESULT_ROOT="${RESULT_ROOT:-${REPO_ROOT}/bench_results/kimi_pd_p1to17d1_mooncake_nsys_autosweep/${RUN_TAG}}"
...
validate_positive_int START_PREFILL_COUNT "$START_PREFILL_COUNT"
validate_positive_int END_PREFILL_COUNT "$END_PREFILL_COUNT"
validate_positive_int CONCURRENCY_PER_PREFILL "$CONCURRENCY_PER_PREFILL"
validate_positive_int PROMPTS_FACTOR "$PROMPTS_FACTOR"
validate_positive_int DECODE_COUNT "$DECODE_COUNT"
...
if (( START_PREFILL_COUNT > END_PREFILL_COUNT )); then
  printf 'error: START_PREFILL_COUNT (%s) must be <= END_PREFILL_COUNT (%s)\n' \
    "$START_PREFILL_COUNT" "$END_PREFILL_COUNT" >&2
  exit 1
fi
...
log "Starting autosweep on partition=${PARTITION}, prefill=${START_PREFILL_COUNT}..${END_PREFILL_COUNT}, decode=${DECODE_COUNT}"
...
for ((prefill_count = START_PREFILL_COUNT; prefill_count <= END_PREFILL_COUNT; prefill_count++)); do
  required_nodes=$((prefill_count + DECODE_COUNT))
  target_concurrency=$((CONCURRENCY_PER_PREFILL * prefill_count))
  num_prompts=$((target_concurrency * PROMPTS_FACTOR))

  export PREFILL_COUNT="$prefill_count"
  export BENCH_MULTI_TURN_CONCURRENCY="$target_concurrency"
  export BENCH_NUM_PROMPTS="$num_prompts"

  point_dir="${RESULT_ROOT}/prefill_${prefill_count}_mtc_${target_concurrency}"
  rendered_config="${point_dir}/rendered_config.yaml"

  export BENCH_RESULT_DIR="$point_dir"
  export BENCH_LABEL="prefill-${prefill_count}-mtc-${target_concurrency}"
  export BENCH_REQUEST_ID_PREFIX="prefill${prefill_count}-mtc${target_concurrency}-"
  ...
  while true; do
    ...
    while true; do
      ...
      if (( idle_nodes >= required_nodes )); then
        log "prefill=${prefill_count} attempt=${attempt}: found ${idle_nodes} usable idle nodes, need ${required_nodes}; launching vigil."
        break
      fi

      idle_list="$(print_idle_nodes)"
      excluded_nodes="$(list_excluded_nodes | tr '\n' ' ')"
      log "prefill=${prefill_count} attempt=${attempt}: only ${idle_nodes} usable idle nodes on partition=${PARTITION}, need ${required_nodes}. Waiting ${WAIT_INTERVAL_SECS}s."
    ...
    log "Running prefill=${prefill_count}, attempt=${attempt}/${MAX_RETRIES}, concurrency=${target_concurrency}, prompts=${num_prompts}"
    ...
    if "$VIGIL_BIN" -c "$rendered_config" --log-dir "$attempt_log_dir" "$@"; then
      ...
      log "Completed prefill=${prefill_count} on attempt=${attempt}"
      attempt_succeeded=1
      break
    else
      exit_code=$?
    ...
    log "prefill=${prefill_count} attempt=${attempt} failed with exit_code=${exit_code}"
    ...
    if (( ${#bad_nodes[@]} > 0 )); then
      log "prefill=${prefill_count} attempt=${attempt}: bad nodes detected; continuing retries without retry cap."
    else
      unknown_failure_count=$((unknown_failure_count + 1))
      log "prefill=${prefill_count} attempt=${attempt}: unknown failure ${unknown_failure_count}/${MAX_RETRIES}."
      if (( unknown_failure_count >= MAX_RETRIES )); then
        log "prefill=${prefill_count}: unknown failures exhausted retry cap; exiting with failure."
        exit "$exit_code"
      fi
    fi

    log "Retrying prefill=${prefill_count} after failure."
    attempt=$((attempt + 1))
  done

  if (( attempt_succeeded == 0 )); then
    log "prefill=${prefill_count} did not complete successfully."
    exit 1
  fi
done
