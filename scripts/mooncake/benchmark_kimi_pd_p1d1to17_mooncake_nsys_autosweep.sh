...
START_DECODE_COUNT="${START_DECODE_COUNT:-3}"
END_DECODE_COUNT="${END_DECODE_COUNT:-17}"
...
RESULT_ROOT="${RESULT_ROOT:-${REPO_ROOT}/bench_results/kimi_pd_p1d1to17_mooncake_nsys_autosweep/${RUN_TAG}}"
...
validate_positive_int PREFILL_COUNT "$PREFILL_COUNT"
validate_positive_int START_DECODE_COUNT "$START_DECODE_COUNT"
validate_positive_int END_DECODE_COUNT "$END_DECODE_COUNT"
...
if (( START_DECODE_COUNT > END_DECODE_COUNT )); then
  printf 'error: START_DECODE_COUNT (%s) must be <= END_DECODE_COUNT (%s)\n' \
    "$START_DECODE_COUNT" "$END_DECODE_COUNT" >&2
  exit 1
fi
...
log "Starting autosweep on partition=${PARTITION}, prefill=${PREFILL_COUNT}, decode=${START_DECODE_COUNT}..${END_DECODE_COUNT}"
...
for ((decode_count = START_DECODE_COUNT; decode_count <= END_DECODE_COUNT; decode_count++)); do
  required_nodes=$((PREFILL_COUNT + decode_count))
  target_concurrency=$((BASE_MULTI_TURN_CONCURRENCY + CONCURRENCY_STEP * (decode_count - 1)))
  num_prompts=$((target_concurrency * PROMPTS_FACTOR))

  export DECODE_COUNT="$decode_count"
  export BENCH_MULTI_TURN_CONCURRENCY="$target_concurrency"
  export BENCH_NUM_PROMPTS="$num_prompts"

  point_dir="${RESULT_ROOT}/decode_${decode_count}_mtc_${target_concurrency}"
  rendered_config="${point_dir}/rendered_config.yaml"

  export BENCH_RESULT_DIR="$point_dir"
  export BENCH_LABEL="prefill-${PREFILL_COUNT}-decode-${decode_count}-mtc-${target_concurrency}"
  export BENCH_REQUEST_ID_PREFIX="p${PREFILL_COUNT}d${decode_count}-mtc${target_concurrency}-"
  ...
  while true; do
    ...
    while true; do
      ...
      if (( idle_nodes >= required_nodes )); then
        log "decode=${decode_count} attempt=${attempt}: found ${idle_nodes} usable idle nodes, need ${required_nodes}; launching vigil."
        break
      fi

      idle_list="$(print_idle_nodes)"
      excluded_nodes="$(list_excluded_nodes | tr '\n' ' ')"
      log "decode=${decode_count} attempt=${attempt}: only ${idle_nodes} usable idle nodes on partition=${PARTITION}, need ${required_nodes}. Waiting ${WAIT_INTERVAL_SECS}s."
    ...
    log "Running decode=${decode_count}, attempt=${attempt}/${MAX_RETRIES}, concurrency=${target_concurrency}, prompts=${num_prompts}"
    ...
    if "$VIGIL_BIN" -c "$rendered_config" --log-dir "$attempt_log_dir" "$@"; then
      ...
      log "Completed decode=${decode_count} on attempt=${attempt}"
      attempt_succeeded=1
      break
    else
      exit_code=$?
    ...
    log "decode=${decode_count} attempt=${attempt} failed with exit_code=${exit_code}"
    ...
    if (( ${#bad_nodes[@]} > 0 )); then
      log "decode=${decode_count} attempt=${attempt}: bad nodes detected; continuing retries without retry cap."
    else
      unknown_failure_count=$((unknown_failure_count + 1))
      log "decode=${decode_count} attempt=${attempt}: unknown failure ${unknown_failure_count}/${MAX_RETRIES}."
      if (( unknown_failure_count >= MAX_RETRIES )); then
        log "decode=${decode_count}: unknown failures exhausted retry cap; exiting with failure."
        exit "$exit_code"
      fi
    fi

    log "Retrying decode=${decode_count} after failure."
    attempt=$((attempt + 1))
  done

  if (( attempt_succeeded == 0 )); then
    log "decode=${decode_count} did not complete successfully."
    exit 1
  fi
done
